...
project_path = os.path.join(workdir, generate_unique_name("pyaedt_sl_diff") + ".aedt")
...
my_mat = q.materials.add_material("my_fr4")
...
obj = [q.modeler.get_object_from_name(i) for i in ["ref_gnd_u", "ref_gnd_l"]]
q.assign_single_conductor(
...
obj = q.modeler.get_object_from_name("signal_p")
q.assign_huray_finitecond_to_edges(obj.edges, radius="$ra", ratio="$sr", name="b_" + obj.name)
